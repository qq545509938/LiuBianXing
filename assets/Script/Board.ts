...
export default class Board extends cc.Component {
    ...
    checkLose() {
        ...
        if (count === 3) {
            const oldScore = cc.sys.localStorage.getItem('bestScore');
            cc.sys.localStorage.setItem('lastScore', theScore);
            if (oldScore < theScore) {
                cc.sys.localStorage.setItem('bestScore', theScore);
            }
            this.gameOver();
        }
    }
    ...
    setHexagonGrid() {
        ...
        for (let q = -this.hexSide; q <= this.hexSide; q++) {
            ...
            let r3 = -q - this.hexSide;
            let r4 = -q + this.hexSide;
            for (let r = r1; r <= r2; r++) {
